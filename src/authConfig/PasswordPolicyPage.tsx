...
export function PasswordPolicyPage() {
  ...
  const handleNumericField = (
    key: "minLength" | "expiryDays" | "sessionTimeoutMinutes" | "maxLoginAttempts" | "lockoutDurationMinutes",
    value: string,
    min: number,
    max: number
  ) => {
    ...
    if (parsed < min || parsed > max) {
      setFieldErrors((e) => ({ ...e, [key]: `Must be between ${min} and ${max}.` }));
      return;
    }
    setFieldErrors((e) => ({ ...e, [key]: "" }));

    if (key === "sessionTimeoutMinutes" || key === "maxLoginAttempts" || key === "lockoutDurationMinutes") {
      patchConfig({ [key]: parsed });
    } else {
      patchConfig({ passwordPolicy: { ...config!.passwordPolicy, [key]: parsed } });
    }
  };
  ...
  const passwordToggles = [
    { key: "requireUppercase" as const, label: "Require Uppercase Letter", description: "At least one uppercase letter (A–Z)" },
    { key: "requireNumbers" as const, label: "Require Number", description: "At least one numeric digit (0–9)" },
    { key: "requireSpecialChars" as const, label: "Require Special Character", description: "At least one special character (!@#$%…)" },
  ];
    ...
    <div className="p-8 max-w-3xl">
      <div className="flex items-start justify-between mb-8">
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-2xl font-black text-text-primary tracking-tight">Password Policy</h1>
          <p className="text-sm text-text-muted mt-1">Define strength and expiry rules for tenant passwords</p>
        </motion.div>
      ...
      <div className="space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          <Card title="Complexity Requirements" subtitle="Minimum requirements for user passwords">
            <div className="space-y-4 mb-6">
              <Input
                label="Minimum Length"
                type="number"
                value={passwordPolicy.minLength}
                onChange={(e) => handleNumericField("minLength", e.target.value, 4, 64)}
            ...
            <div className="space-y-4">
              {passwordToggles.map((toggle) => (
                <Toggle
                  ...
                  onChange={(v) => patchConfig({ passwordPolicy: { ...passwordPolicy, [toggle.key]: v } })}
                  label={toggle.label}
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <Card title="Password Expiry" subtitle="Force periodic password resets">
            <Input
              label="Expiry (days)"
              type="number"
              value={passwordPolicy.expiryDays}
              onChange={(e) => handleNumericField("expiryDays", e.target.value, 0, 365)}
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
          <Card title="Session & Lockout Rules" subtitle="Control session lifetime and brute-force protection">
            <div className="space-y-4">
              <Input
                label="Session Timeout (minutes)"
                type="number"
                value={config.sessionTimeoutMinutes}
                onChange={(e) => handleNumericField("sessionTimeoutMinutes", e.target.value, 5, 1440)}
              ...
              <Input
                ...
                onChange={(e) => handleNumericField("maxLoginAttempts", e.target.value, 1, 20)}
              ...
              <Input
                ...
                onChange={(e) => handleNumericField("lockoutDurationMinutes", e.target.value, 1, 1440)}

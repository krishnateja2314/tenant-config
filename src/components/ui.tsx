...
export function Button({
  ...
  const variants = {
    primary: "bg-accent text-white hover:bg-accent-hover active:scale-[0.98] shadow-glow",
    secondary: "bg-surface-2 text-text-primary border border-border hover:border-accent/50 active:scale-[0.98]",
...
export function SectionDivider({ label }: { label: string }) {
  return (
    <div className="flex items-center gap-3 my-2">
      ...
      <span className="text-xs font-semibold uppercase tracking-widest text-text-muted/60">{label}</span>
      <div className="flex-1 h-px bg-border" />
    </div>
  );
}
